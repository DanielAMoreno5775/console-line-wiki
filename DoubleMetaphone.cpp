...
void DoubleMetaphone::MakeUpper(string &s) {
	for (unsigned int i = 0; i < s.length(); i++) {
		s[i] = toupper(s[i]);
	}
}

int DoubleMetaphone::IsVowel(string &s, int pos) {
	char c = '\0';
...
int DoubleMetaphone::SlavoGermanic(string &s) {
...
char DoubleMetaphone::GetAt(string &s, int pos) {
	if ((pos < 0) || (pos >= s.length())) {
		return '\0';
	}
...
void DoubleMetaphone::SetAt(string &s, int pos, char c) {
	if ((pos < 0) || (pos >= s.length())) {
		return;
	}
...
*/
int DoubleMetaphone::StringAt(string &s, int start, int length, ...) {
	char *test = nullptr;
	const char *pos = (s.c_str() + start);
	va_list ap;
...
DoubleMetaphone::DoubleMetaphone(const string &str, vector<string> *codes) {
	int length = 0;
	string original = "";
	string primary = "";
	string secondary = "";
